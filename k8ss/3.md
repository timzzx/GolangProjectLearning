...
## kubectl apply
apply通过定义kubernetes资源的文件来管理应用。它通过运行kubectl apply在集群中创建和更新资源。这是在生产中管理kubernetes应用的推荐方法
...
## 查看和查找资源
```shell
kubectl get services # 列出当前命名空间下所有的service
